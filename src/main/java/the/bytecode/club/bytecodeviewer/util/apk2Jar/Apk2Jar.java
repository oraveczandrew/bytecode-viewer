...
public abstract class Apk2Jar
{
    ...
    final public ResourceContainer importFromApk(File inputApk) throws IOException {
        synchronized (workLock) {
            return importFromApkImpl(inputApk);
        }
    }

    protected abstract ResourceContainer importFromApkImpl(File inputApk) throws IOException;
    ...
    final public File apk2Jar(File input) {
        ...
        apk2Jar(input, output);
        return output;
    }

    final public void apk2Jar(File input, File output) {
        synchronized (workLock) {
